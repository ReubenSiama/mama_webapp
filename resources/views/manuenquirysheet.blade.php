		...
		<div class="panel panel-primary">
			<div class="panel-heading text-center">
                       <form action="{{ URL::to('/') }}/subward" method="post">
                       	{{ csrf_field() }}
                       	 <input type="text" name="wardid" placeholder="Subward No" style="color:red;">
                       	 <input type="text" name="manu_id" placeholder="Manufacturer Id" style="color:red;">
			...
			@endif
				...
				<table id="myTable" class="table table-responsive table-striped table-hover">
					...
					<tbody>
                        ...
                        @if($enquiry->status != "Not Processed")
					        ...
					        @if($enquiry->manu_id == NULL)
                                <a href="{{ URL::to('/')}}/viewsubward?pr<tbody>
						<?php $pro=0; $con=0; $total=0; $sum=0; $sum1=0; $sum2=0; ?>
						@foreach($enquiries as $enquiry)
                       ...
                       {{ $enquiry->proc != null ? $enquiry->proc->name :''  }}
							...
							<td style="text-align: center">{{$enquiry->total_quantity * $enquiry->price}}</td>
					...
					</tbody>ojectid={{$enquiry -> project_id}} && subward={{ $ward->sub_ward_name }}" target="_blank">
                                    {{$ward->sub_ward_name}}
