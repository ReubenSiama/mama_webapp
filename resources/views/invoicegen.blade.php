...
@section('content')
  ...
  <span class="pull-right"> @include('flash-message')</span>
      ...
      <form  id="yes" action="{{URL::to('/')}}/sdhg" method="POST">
   ...
   <td>Billing Address : </td> 
      <td><textarea  required type="text" name="bill" class="form-control" style="resize: none;" rows="5">{{$bill}}</textarea></td>
  </tr>
  <tr>
 ...
 <!--  <tr>
   <td>HSN: </td> 
      <td><input type="text" name="hsn" value="" class="form-control" >  </td>
  </tr> -->
    ...
    <tbody>
        ...
        <tr>
         ...
         <input type="hidden" name="ids[]" value="{{$pp->id}}">
            <td >
                <?php $ids = [40,41,52,48];
                 $categories = App\Category::whereIn('id',$ids)->get(); ?>
                       <select id="category2156{{$pp->id}}" onchange="brands('{{$pp->id}}')" class="form-control" name="cat255[]" readonly >
            ...
            <td> <select id="brands24{{$pp->id}}"  class="form-control" name="brand55[]" readonly>
            ...
            <input type="hidden" name="unitprice1[]" id="unitprice1{{$pp->id}}" readonly value="{{$pp->unitprice}}">
            <td >
               <textarea class="form-control" name="desc1[]" readonly>{{$pp->description}}</textarea>
            </td>
           <td>
                   <input type="text"  name="hsn1[]" class="form-control"  id="hsn" value="{{$pp->hsn}}" readonly>
             </td>
            
             <td>
                   <input type="text"  name="quan1[]" class="form-control" id="quan8900{{$pp->id}}" onkeyup="getgst1('{{$pp->id}}')" value="{{$pp->quan}}" readonly>
             </td>
             <td>
                   <input type="text"  name="price1[]" class="form-control" step="0.01" id="price8900{{$pp->id}}" onkeyup="getgst1('{{$pp->id}}')" value="{{$pp->price}}" readonly> 
             </td>
             <td>
                    <?php $statef = App\Category::distinct()->get(['measurement_unit']); ?>
                            <select  name="unit1[]" class="form-control" readonly>
                              <option>--select--</option>
                              @foreach($statef as $state)
             ...
             <td>
                    <?php $states = App\State::all(); ?>
                            <select  name="state12[]" class="form-control" onchange="getgst1('{{$pp->id}}')" id="state8900{{$pp->id}}" readonly>
                              <option>--select--</option>
                              @foreach($states as $state)
               ...
               <td>
                   <input type="text" readonly name="gst1[]" class="form-control" step="0.01" id="gst1{{$pp->id}}" value="{{$pp->gst}}" readonly>
                  <?php array_push($totalgsts, $pp->gst); ?>
             </td>
               <td>
                   <input type="text" readonly name="withgst1[]" class="form-control" step="0.01" id="withgst1{{$pp->id}}" value="{{$pp->withgst}}" readonly>
                  <?php array_push($withgsts, $pp->withgst); ?>

             </td>
               <td>
                   <input type="text" readonly name="withoutgst1[]" class="form-control" step="0.01" id="withoutgst1{{$pp->id}}" value="{{$pp->withoutgst}}" readonly>
                  <?php array_push($withoutgst1s, $pp->withoutgst); ?>
...
@endif
 <?php $mmm = [40,41,52,48]; $eqs = App\FLOORINGS::where('req_id',$req_id)->whereIn('category',$mmm)->first(); ?>
     ...
     <div class="container">
       <div class="col-md-3 col-md-offset-4">
        <!-- <center>  <button class="btn-sm btn btn-warning" type="submit">Get Total Details</button></center> -->
         <label>Total Gst Amount <input type="text" name="totalgst" class="form-control" id="totalgst" value="{{array_sum($totalgsts)}}"></label>
         <label>Total WithGST Amount <input type="text" name="totalwithgst" class="form-control" id="totalwithgst"  value="{{array_sum($withgsts)}}"></label>
          <label>Total WithOutGst Amount<input type="text" name="totalwithoutgst" class="form-control" id="totalwithoutgst" value="{{array_sum($withoutgst1s)}}"></label>
  ...
  border-left: 6px solid #ffeb3b;text-align:center;">
  <p style="font-weight:bold;">Any changes required  please go to Enquiry Edit page And  chage the values !</p>
</div>
        
        <center>  <button type="button" class="btn btn-sm btn-warning" onclick="submitfrom()" >Submit The Form Get Invoice </button></center>
      </form>
</div>
<script type="text/javascript">
