...
?>
                    <td id="projname-{{$project->project_id}}"> {{ $project->procurementdetails->procurement_name ?? '' }}</td>
                                    <td  style="text-align:center"><a href="{{ URL::to('/') }}/admindailyslots?projectId={{$project->project_id}}" target="_blank">{{ $project->project_id }}</a></td>
                    <td id="projsite-{{$project->project_id}}">
                                     <a target="_blank" href="https://maps.google.co.in?q={{ $project->siteaddress->address ?? '' }}">{{ $project->siteaddress->address ?? '' }}</a>
                                    </td>
                    <td id="projproc-{{$project->project_id}}">
                                        {{ $project->procurementdetails->procurement_name ?? '' }}
                                    </td>
                    <td id="projcont-{{$project->project_id}}"><address>{{ $project->procurementdetails->procurement_contact_no ?? '' }}</address></td>
...
<td>
  <?php $last = App\UpdatedReport::where('project_id',$project->project_id)->get()->last(); 
  ?>
  @if(count($last) > 0)
{{ date('d-m-Y', strtotime( $last->updated_at)) }}<br>
{{ date('h:i:s A', strtotime($last->updated_at)) }}<br>
{{$last->updateuser != null ? $last->updateuser->name : '' }}
@endif
  </td>
