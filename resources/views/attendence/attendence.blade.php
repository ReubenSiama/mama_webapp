...
<div class="col-md-12 col-sm-12">
	...
	<div class="panel-heading ">
        ...
        <div class="row">
        	<form action="{{URL::to('/')}}/attendence" method="get">
									{{ csrf_field() }}
               ...
               <div class="col-sm-2">
               <br>	<button type="submit" class="btn btn-sm btn-primary">Get Attendence Details</button>
               </div>
           </form>
			</div>
  ...
  <div class="row">
    ...
    @foreach($attendence as $df)
   <div class="col-sm-4">
  <div class="card card-default">
  
  <form action="{{URL::to('/')}}/attendenceapproval" method="post" id="yes{{$df['full']->id}}">
									{{ csrf_field() }}
  <input type="hidden" name="userid" value="{{$df['userid']}}">
  <input type="hidden" name="logindate" value="{{$df['full']->logindate}}">
  <input type="hidden" name="loginid" value="{{$df['full']->id}}">
   <div class="card-header" style="background-color:#ccafaf;">
     <?php $user = App\User::where('id',$df['userid'])->pluck('name')->first(); ?>
  ...
  <div class="card-body card-5-7">
    ...
    <div class="card-right">
      ...
      <tr style="background-color:#0cd988;">
      	<td>{{$df['full']->logintime}}</td>
      	<td class="price"><input type="text" name="totalthours" class="form-control" value="{{$df['full']->logout}}"></td>

      	<td class="note" >
          @if($df['total'] > 0)           
      	  <input type="text" name="totalthours" class="form-control" value="{{round($df['total'],2)}}"> 
      	  @else
      	  <input type="text" name="totalthours" class="form-control" value="He is Not Logout Day"> 
      ...
      @else
       <tr>
          ...
          @if($df['total'] > 0)
      	   <input type="text" name="totalthours" class="form-control" value="{{round($df['total'],2)}}"> 
      	   @else
      	  <input type="text" name="totalthours" class="form-control" value=""> 
         @endif
      </td>
  ...
  <div class="card-footer" style="background-color:#ccafaf;">
    <center><center>
  	<button onclick="submit('{{$df['full']->id}}')" class="btn btn-sm btn-success">Approve</button></center></center>
  </div>
</form>
</div>
