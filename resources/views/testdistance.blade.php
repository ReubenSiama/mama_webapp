...
<div class="container">
  ...
  <span class="pull-right"> @include('flash-message')</span>
        ...
        <form method="get" onsubmit="validateform()" id="km" action="{{ URL::to('/') }}/testdistance" enctype="multipart/form-data">
  ...
  @if(count($projects) > 0))
    ...
    @foreach($projects as $project)
     <?php $x=explode(",",$project->projectdetails->image ?? ""); 
          
    ?>
      locations.push(["<div class=card style=width:350px>  <img src={{ $url}}/projectImages/{{ $x[0] }}   style=width:200px height=200px><div class=container><h4>project_id Id : <b><a target=_blank href={{URL::to('/')}}/showThisProject?id={{$project->project_id}}><b>{{$project->project_id }}  </b></a></h4></h4><p><a href=\"https://maps.google.com/?q={{ $project->address }}\">Address: {{ $project->address }}</p></a></div></div>",{{ $project->latitude}}, {{ $project->longitude }}]);
